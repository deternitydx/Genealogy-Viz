...
SELECT 
  ...
  P1.\"Surname\", 
  P1.\"GivenName\", 
  P1.\"BirthDateSearchable\", 
  P1.\"DeathDateSearchable\",
  P2.\"DeathDateSearchable\" as \"WifeDeathDate\"
FROM 
  public.\"Marriage\", 
  public.\"Person\" as P1,
  public.\"Person\" as P2
WHERE 
  \"Marriage\".\"HusbandID\" = P1.\"ID\" AND \"Marriage\".\"WifeID\" = P2.\"ID\"
...
foreach ($arr as $i => $mar) {
	...
	if ($i > 0 && $arr[$i-1]["HusbandID"] == $mar["HusbandID"] && $mar["MarriageDateSearchable"] != "" && $mar["MarriageDateSearchable"] < "1845-12-10"
			//&& $arr[$i-1]["WifeDeathDate"] < "1845-12-10") { // and previous wife's death date is before the temple
			&& $arr[$i-1]["WifeDeathDate"] != "" && $arr[$i-1]["WifeDeathDate"] < $mar["MarriageDateSearchable"]) { // and previous wife's death date is before this marraige
		array_push($plural, $arr[$i-1]);
	}
}


foreach ($plural as $i => $mar) {
	$resa = array();
